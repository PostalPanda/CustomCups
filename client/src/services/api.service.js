...
//     : "https://api.degencups.io/api";
export const API_URL ="https://api.degencups.io/api";
...
export const RECAPTCHA_SITE_KEY = "6LdnqbIhAAAAALvLF_vZ-dosuxi9xlvyl9JOmqM0";
...
// Register api
export const tryRegisterUser = async (username, password, email) => {
  return new Promise(async (resolve, reject) => {
    try {
...
// Deposit api
export const depositSol = async data => {
  return new Promise(async (resolve, reject) => {
    try {
      if (!data.amount || data.amount <= 0 || isNaN(data.amount)) return;
